	...
	return new Promise((resolve, reject) => {
		try {
			...
			if (response.Success) {
				...
				reject(
					`Detection failed. StatusCode: ${response.StatusCode}, StatusMessage: ${response.StatusMessage}`,
				);
			}
		} catch (error) {
			reject(error);
	...
	return new Promise((resolve, reject) => {
		try {
			...
			if (response.Success) {
				...
				reject(
					`Translation failed. StatusCode: ${response.StatusCode}, StatusMessage: ${response.StatusMessage}`,
				);
			}
		} catch (error) {
			reject(error);
